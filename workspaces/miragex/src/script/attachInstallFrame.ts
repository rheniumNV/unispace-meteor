...
export const attachInstallFrame = async (option: {
  ...
  if (frameCode !== "InstalledAvatar") {
    console.warn(`FrameCode is not InstalledAvatar. FrameCode: ${frameCode}`);
    return;
  }
  ...
  console.info("attached to install frame");
};

attachInstallFrame({
  inputPath: path.resolve(__dirname, ""),
  outputPath: path.resolve(__dirname, "../res/frame/installFrame/"),
});
